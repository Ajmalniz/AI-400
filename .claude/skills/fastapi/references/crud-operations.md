# Full CRUD Operations Reference

Complete guide to implementing Create, Read, Update, Delete operations in FastAPI.
...
But agents rarely want _everything_. Add filtering:
...
**But wait—isn't that PATCH behavior?** Yes, this is actually partial update logic. True PUT would require ALL fields. In practice, most APIs use PUT for what should be PATCH because PATCH had browser support issues historically. You'll see both patterns.
...
Some APIs return 204 No Content for deletes. We're returning a confirmation message, which helps with debugging and agent feedback.
...
**Problem 1: No atomic updates.** If two requests try to update the same task simultaneously, they might both read the old value, make changes, and write—one update overwrites the other.

**Problem 2: No persistence.** Restart the server, and all changes are lost.

**Problem 3: Not thread-safe.** With multiple workers, shared state can corrupt.

**Problem 4: No transactions.** If updating a task should also update related data, there's no way to ensure both succeed or both fail together.

For learning, this is fine. For production agent systems, you'll use databases with proper concurrency control, transactions, and persistence. But understanding the pattern matters—the CRUD operations stay the same, only the storage mechanism changes.

## Complete Implementation

Here's the full Task API with all CRUD operations:
...
**Mistake 4: Not using query parameters for filtering**

```python
# Wrong - requires separate endpoints
...
- Use GET for reading (list all or single resource)
- Use POST for creating new resources
- Use PUT for updating resources (often used for partial updates in practice)
- Use DELETE for removing resources
- Always return appropriate HTTP status codes (200, 201, 204, 404)
- Handle not-found cases with HTTPException(status_code=404)
- Return updated resources after PUT operations
- Use query parameters for filtering list endpoints
- Understand HTTP method semantics (safe, idempotent)
- These patterns transfer directly to agent APIs that manage state
